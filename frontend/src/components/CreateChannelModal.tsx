...
import type { User, Channel } from "stream-chat";
...
const CreateChannelModal: React.FC<CreateChannelModalProps> = ({ onClose }) => {
    ...
    const fetchUsers = async (): Promise<void> => {
      ...
      try {
        const response = await client.queryUsers(
          { id: { $ne: client.user.id } } as Record<string, unknown>,
          { name: 1 },
          { limit: 100 }
        );
  ...
  const handleMemberToggle = (id: string): void => {
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      const channel: Channel = client.channel(
        "messaging",
        channelId,
        channelData
      );
